...
import (
	...
	milkywayerrors "github.com/milkyway-labs/milkyway/v4/types/errors"
)
...
func (g GovVoteDecorator) ValidateVoteMsgs(ctx sdk.Context, msgs []sdk.Msg) error {
	validMsg := func(m sdk.Msg) error {
		...
		if !enoughStake {
			return errorsmod.Wrapf(milkywayerrors.ErrInsufficientStake, "insufficient stake for voting - min required %v", minStakedTokens)
		}
	...
	validAuthz := func(execMsg *authz.MsgExec) error {
		for _, v := range execMsg.Msgs {
			...
			if err := g.cdc.UnpackAny(v, &innerMsg); err != nil {
				return errorsmod.Wrap(milkywayerrors.ErrUnauthorized, "cannot unmarshal authz exec msgs")
			}
