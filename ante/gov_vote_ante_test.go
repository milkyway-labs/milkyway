...
import (
	...
	"github.com/milkyway-labs/milkyway/v2/ante"
	"github.com/milkyway-labs/milkyway/v2/app/helpers"
)
...
func TestVoteSpamDecoratorGovV1Beta1(t *testing.T) {
	milkyWayApp := helpers.Setup(t)
	ctx := milkyWayApp.NewUncachedContext(true, tmproto.Header{})
	decorator := ante.NewGovVoteDecorator(milkyWayApp.AppCodec(), milkyWayApp.StakingKeeper)
	...
	addr, err := milkyWayApp.AccountKeeper.Accounts.Indexes.Number.MatchExact(ctx, 0)
	require.NoError(t, err)
