...
func TestGovExpeditedProposalsDecorator(t *testing.T) {
	...
	testCases := []struct {
		...
		// submitted using "milkywayd tx gov submit-legacy-proposal"
		{
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			txCfg := milkyWayApp.GetTxConfig()
			decorator := ante.NewGovExpeditedProposalsDecorator(milkyWayApp.AppCodec())
