...
func (s *KeeperTestSuite) TestMintDenomMsg() {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			amount:    10,
			mintDenom: "factory/milk1t7egva48prqmzl59x5ngv4zx0dtrwewcrntvt4/evmos",
...
func (s *KeeperTestSuite) TestBurnDenomMsg() {
	...
	for _, tc := range []struct {
		...
		{
			desc:      "denom does not exist",
			burnDenom: "factory/milk1t7egva48prqmzl59x5ngv4zx0dtrwewcrntvt4/evmos",
			admin:     s.TestAccs[0].String(),
			valid:     false,
		},
		{
...
func (s *KeeperTestSuite) TestForceTransferMsg() {
	...
	s.Run(fmt.Sprintf("test force transfer"), func() {
		...
		govModAcc := s.App.AccountKeeper.GetModuleAccount(s.Ctx, minttypes.ModuleName)
		...
		s.Require().ErrorContains(err, "send from blocked addr not available")
	})
}
