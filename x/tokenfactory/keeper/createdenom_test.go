...
func (s *KeeperTestSuite) TestMsgCreateDenom() {
	var (
		...
		denomCreationFee = sdk.NewCoins(sdk.NewCoin(appparams.BaseCoinUnit, sdkmath.NewInt(1000000)))
	)

	// Set the denom creation fee. It is currently turned off in favor
	// of gas charge by default.
	params, err := s.App.TokenFactoryKeeper.GetParams(s.Ctx)
	...
	postCreateBalance := bankKeeper.GetBalance(s.Ctx, s.TestAccs[0], params.DenomCreationFee[0].Denom)
...
func (s *KeeperTestSuite) TestCreateDenom() {
	...
	for _, tc := range []struct {
		...
		s.Run(fmt.Sprintf("Case %s", tc.desc), func() {
			...
			s.Require().NoError(err)
			params, err := tokenFactoryKeeper.GetParams(s.Ctx)
			s.Require().NoError(err)
