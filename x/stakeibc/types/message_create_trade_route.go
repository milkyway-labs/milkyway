...
func (msg *MsgCreateTradeRoute) ValidateBasic() error {
	...
	maxAllowedSwapLossRate, err := sdkmath.LegacyNewDecFromStr(msg.MaxAllowedSwapLossRate)
	if err != nil {
		return errorsmod.Wrapf(err, "unable to cast max allowed swap loss rate to a decimal")
	}
	if maxAllowedSwapLossRate.LT(sdkmath.LegacyZeroDec()) || maxAllowedSwapLossRate.GT(sdkmath.LegacyOneDec()) {
