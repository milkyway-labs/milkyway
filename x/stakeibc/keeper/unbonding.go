...
func (k Keeper) ConsolidateUnbondingMessages(
	...
	totalRemainingDelegationsAcrossBatch := sdkmath.LegacyZeroDec()
	remainingDelegationsInBatchByVal := map[string]sdkmath.LegacyDec{}
	for _, capacity := range unbondCapacities {
		...
		if initialUnbondAmount, ok := initialUnbondAmountFromBatchByVal[capacity.ValidatorAddress]; ok {
			remainingDelegation := sdkmath.LegacyNewDecFromInt(capacity.CurrentDelegation.Sub(initialUnbondAmount))
	...
	if sdkmath.LegacyNewDecFromInt(totalExcessAmount).GT(totalRemainingDelegationsAcrossBatch) {
	...
	for i := range unbondingsBatch {
		...
		if i != len(unbondingsBatch)-1 {
			...
			validatorUnbondIncrease = sdkmath.LegacyNewDecFromInt(totalExcessAmount).Mul(unbondIncreaseProportion).TruncateInt()
			...
			if sdkmath.LegacyNewDecFromInt(excessRemaining).GT(remainingDelegation) {
