...
func (k Keeper) BuildSwapMsg(rewardAmount sdkmath.Int, route types.TradeRoute) (msg types.MsgSwapExactAmountIn, err error) {
	...
	rewardAmountConverted := sdkmath.LegacyNewDecFromInt(rewardAmount).Mul(tradeConfig.SwapPrice)
	minOutPercentage := sdkmath.LegacyOneDec().Sub(tradeConfig.MaxAllowedSwapLossRate)
...
func (k Keeper) TradeRewardBalanceQuery(ctx sdk.Context, route types.TradeRoute) error {
	...
	queryData := append(v3.CreateAccountBalancesPrefix(tradeAddressBz), []byte(route.RewardDenomOnTradeZone)...)
...
func (k Keeper) TradeConvertedBalanceQuery(ctx sdk.Context, route types.TradeRoute) error {
	...
	queryData := append(v3.CreateAccountBalancesPrefix(tradeAddressBz), []byte(route.HostDenomOnTradeZone)...)
