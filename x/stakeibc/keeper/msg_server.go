...
func (k msgServer) RegisterHostZone(goCtx context.Context, msg *types.MsgRegisterHostZone) (*types.MsgRegisterHostZoneResponse, error) {
	...
	if err := k.ValidateAdminAddress(ctx, msg.Creator); err != nil {
		return nil, err
	}
	...
	if msg.MinRedemptionRate.IsNil() || msg.MinRedemptionRate.IsZero() {
		msg.MinRedemptionRate = sdkmath.LegacyNewDecWithPrec(int64(params.DefaultMinRedemptionRateThreshold), 2)
	}
	if msg.MaxRedemptionRate.IsNil() || msg.MaxRedemptionRate.IsZero() {
		msg.MaxRedemptionRate = sdkmath.LegacyNewDecWithPrec(int64(params.DefaultMaxRedemptionRateThreshold), 2)
	}
...
func (ms msgServer) CreateTradeRoute(goCtx context.Context, msg *types.MsgCreateTradeRoute) (*types.MsgCreateTradeRouteResponse, error) {
	...
	tradeConfig := types.TradeConfig{
		PoolId:               msg.PoolId,
		SwapPrice:            sdkmath.LegacyZeroDec(), // this should only ever be set by ICQ so initialize to blank
		PriceUpdateTimestamp: 0,

		MaxAllowedSwapLossRate: sdkmath.LegacyMustNewDecFromStr(maxAllowedSwapLossRate),
