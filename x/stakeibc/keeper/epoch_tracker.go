...
func (k Keeper) GetStrideEpochElapsedShare(ctx sdk.Context) (sdkmath.LegacyDec, error) {
	...
	if !found {
		...
		return sdkmath.LegacyZeroDec(), errorsmod.Wrapf(sdkerrors.ErrNotFound, errMsg)
	}
	...
	if err != nil {
		...
		k.Logger(ctx).Error(errMsg)
		return sdkmath.LegacyZeroDec(), errorsmod.Wrapf(types.ErrIntCast, errMsg)
	}
	...
	if currBlockTime < epochStartTime || currBlockTime > epochEndTime {
		...
		k.Logger(ctx).Error(errMsg)
		return sdkmath.LegacyZeroDec(), errorsmod.Wrapf(types.ErrInvalidEpoch, errMsg)
	}
