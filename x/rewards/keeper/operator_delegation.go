...
func (k *Keeper) initializeOperatorDelegation(ctx context.Context, operatorID uint32, del string) error {
	...
	err = k.incrementOperatorReferenceCount(ctx, operatorID, previousPeriod)
	if err != nil {
		return err
	}
	...
	stake := operator.TokensFromSharesTruncated(delegation.Shares)
	return k.OperatorDelegatorStartingInfos.Set(
		ctx, collections.Join(operatorID, del),
		types.NewMultiDelegatorStartingInfo(previousPeriod, stake, uint64(sdkCtx.BlockHeight())))
}

// calculate the rewards accrued by a delegation between two periods
func (k *Keeper) calculateOperatorDelegationRewardsBetween(ctx context.Context, operator operatorstypes.Operator,
	...
	starting, err := k.OperatorHistoricalRewards.Get(ctx, collections.Join(operator.ID, startingPeriod))
	if err != nil {
		return nil, err
	}

	ending, err := k.OperatorHistoricalRewards.Get(ctx, collections.Join(operator.ID, endingPeriod))
	if err != nil {
		return nil, err
	}
...
func (k *Keeper) CalculateOperatorDelegationRewards(ctx context.Context, operator operatorstypes.Operator, del restakingtypes.Delegation, endingPeriod uint64) (rewards types.DecPools, err error) {
	...
	currentStakes := operator.TokensFromShares(del.Shares)
	...
	for i, stake := range stakes {
		...
		if stake.Amount.GT(currentStake) {
			...
			//                   calculations across operator's distribution periods
			//
	...
	delRewards, err := k.calculateOperatorDelegationRewardsBetween(ctx, operator, startingPeriod, endingPeriod, stakes)
	if err != nil {
		return nil, err
	}
...
func (k *Keeper) withdrawOperatorDelegationRewards(ctx context.Context, operator operatorstypes.Operator, del restakingtypes.Delegation) (types.Pools, error) {
	...
	endingPeriod, err := k.IncrementOperatorPeriod(ctx, operator)
	if err != nil {
		return nil, err
	}

	rewardsRaw, err := k.CalculateOperatorDelegationRewards(ctx, operator, del, endingPeriod)
	if err != nil {
		return nil, err
	}

	outstanding, err := k.GetOperatorOutstandingRewardsCoins(ctx, operator.ID)
	if err != nil {
		return nil, err
	}
	...
	if !rewards.IsEqual(rewardsRaw) {
		logger := k.Logger(ctx)
		logger.Info(
			"rounding error withdrawing rewards from operator",
	...
	err = k.OperatorOutstandingRewards.Set(ctx, operator.ID, types.MultiOutstandingRewards{Rewards: outstanding.Sub(rewards)})
	if err != nil {
		return nil, err
	}
	...
	startingPeriod := startingInfo.PreviousPeriod
	err = k.decrementOperatorReferenceCount(ctx, operator.ID, startingPeriod)
	if err != nil {
		return nil, err
	}
