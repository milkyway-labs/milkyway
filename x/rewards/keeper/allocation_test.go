...
func (suite *KeeperTestSuite) TestAllocateRewards_InactivePlan() {
	...
	suite.Ctx = suite.Ctx.WithBlockTime(time.Date(2023, 6, 1, 0, 0, 0, 0, time.UTC))
	...
	suite.allocateRewards(3 * time.Second)

	rewards, err := suite.keeper.GetServiceDelegationRewards(suite.Ctx, delAddr, service.ID)
...
func (suite *KeeperTestSuite) TestAllocateRewards_BasicScenario() {
	...
	rewards, err := suite.keeper.GetPoolDelegationRewards(suite.Ctx, aliceAddr, 1)
	suite.Require().NoError(err)
	suite.Require().Equal("4061.052631578900000000service1,25161.000000000000000000service2", rewards.Sum().String())
	rewards, err = suite.keeper.GetPoolDelegationRewards(suite.Ctx, aliceAddr, 2)
	suite.Require().NoError(err)
	suite.Require().Equal("6091.578947368400000000service1", rewards.Sum().String())
	rewards, err = suite.keeper.GetPoolDelegationRewards(suite.Ctx, aliceAddr, 3)
	suite.Require().NoError(err)
	...
	rewards, err = suite.keeper.GetServiceDelegationRewards(suite.Ctx, bobAddr, service1.ID)
	suite.Require().NoError(err)
	suite.Assert().Equal("6091.578947368400000000service1", rewards.Sum().String())
	rewards, err = suite.keeper.GetServiceDelegationRewards(suite.Ctx, bobAddr, service2.ID)
	suite.Require().NoError(err)
	suite.Assert().Equal("75483.000000000000000000service2", rewards.Sum().String())
	rewards, err = suite.keeper.GetServiceDelegationRewards(suite.Ctx, bobAddr, service3.ID)
	suite.Require().NoError(err)
	...
	rewards, err = suite.keeper.GetOperatorDelegationRewards(suite.Ctx, charlieAddr, operator1.ID)
	suite.Require().NoError(err)
	suite.Assert().Equal("18274.736842105000000000service1,113224.500000000000000000service2", rewards.Sum().String())
	rewards, err = suite.keeper.GetOperatorDelegationRewards(suite.Ctx, charlieAddr, operator2.ID)
	suite.Require().NoError(err)
	suite.Assert().Equal("19290.000000000000000000service1,305426.847222222000000000service3", rewards.Sum().String())
	rewards, err = suite.keeper.GetOperatorDelegationRewards(suite.Ctx, charlieAddr, operator3.ID)
	suite.Require().NoError(err)
	...
	rewards, err = suite.keeper.GetPoolDelegationRewards(suite.Ctx, davidAddr, 1)
	suite.Require().NoError(err)
	suite.Assert().Equal("8122.105263157800000000service1,50322.000000000000000000service2", rewards.Sum().String())

	rewards, err = suite.keeper.GetPoolDelegationRewards(suite.Ctx, davidAddr, 2)
	suite.Require().NoError(err)
	suite.Assert().Equal("12183.157894736800000000service1", rewards.Sum().String())

	rewards, err = suite.keeper.GetPoolDelegationRewards(suite.Ctx, davidAddr, 3)
	suite.Require().NoError(err)
	suite.Assert().Equal("4061.052631578800000000service1,25161.000000000000000000service2", rewards.Sum().String())

	rewards, err = suite.keeper.GetServiceDelegationRewards(suite.Ctx, davidAddr, service1.ID)
	suite.Require().NoError(err)
	suite.Assert().Equal("8122.105263157800000000service1", rewards.Sum().String())

	rewards, err = suite.keeper.GetServiceDelegationRewards(suite.Ctx, davidAddr, service2.ID)
	suite.Require().NoError(err)
	suite.Assert().Equal("50322.000000000000000000service2", rewards.Sum().String())

	rewards, err = suite.keeper.GetServiceDelegationRewards(suite.Ctx, davidAddr, service3.ID)
	suite.Require().NoError(err)
	suite.Assert().Equal("128600.777777777600000000service3", rewards.Sum().String())

	rewards, err = suite.keeper.GetOperatorDelegationRewards(suite.Ctx, davidAddr, operator1.ID)
	suite.Require().NoError(err)
	suite.Assert().Equal("7309.894736842000000000service1,45289.800000000000000000service2", rewards.Sum().String())

	rewards, err = suite.keeper.GetOperatorDelegationRewards(suite.Ctx, davidAddr, operator2.ID)
	suite.Require().NoError(err)
	suite.Assert().Equal("7716.000000000000000000service1,122170.738888888800000000service3", rewards.Sum().String())

	rewards, err = suite.keeper.GetOperatorDelegationRewards(suite.Ctx, davidAddr, operator3.ID)
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestAllocateRewards_MovingPrice() {
	...
	rewards, err := suite.keeper.GetServiceDelegationRewards(suite.Ctx, delAddr1, service.ID)
	suite.Require().NoError(err)
	suite.Require().Equal("4629.600000000000000000service", rewards.Sum().String())
	rewards, err = suite.keeper.GetServiceDelegationRewards(suite.Ctx, delAddr2, service.ID)
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestAllocateRewards_ZeroDelegations() {
	...
	target, err := suite.keeper.GetDelegationTarget(suite.Ctx, restakingtypes.DELEGATION_TYPE_OPERATOR, operator.ID)
	suite.Require().NoError(err)
	rewards, err := suite.keeper.GetOutstandingRewardsCoins(suite.Ctx, target)
	suite.Require().NoError(err)
	suite.Require().Empty(rewards)

	target, err = suite.keeper.GetDelegationTarget(suite.Ctx, restakingtypes.DELEGATION_TYPE_SERVICE, service.ID)
	suite.Require().NoError(err)
	rewards, err = suite.keeper.GetOutstandingRewardsCoins(suite.Ctx, target)
	suite.Require().NoError(err)
	suite.Require().Empty(rewards)
	...
	rewards, err = suite.keeper.GetPoolDelegationRewards(suite.Ctx, delAddr1, 1)
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestAllocateRewards_WeightedDistributions() {
	...
	rewards, err = suite.keeper.GetPoolDelegationRewards(suite.Ctx, delAddr2, 1)
	suite.Require().NoError(err)
	...
	rewards, err = suite.keeper.GetPoolDelegationRewards(suite.Ctx, delAddr3, 2)
	suite.Require().NoError(err)
	...
	rewards, err = suite.keeper.GetPoolDelegationRewards(suite.Ctx, delAddr4, 2)
	suite.Require().NoError(err)
	...
	rewards, err = suite.keeper.GetOperatorDelegationRewards(suite.Ctx, delAddr5, operator1.ID)
	suite.Require().NoError(err)
	...
	rewards, err = suite.keeper.GetOperatorDelegationRewards(suite.Ctx, delAddr6, operator1.ID)
	suite.Require().NoError(err)
	...
	rewards, err = suite.keeper.GetOperatorDelegationRewards(suite.Ctx, delAddr7, operator2.ID)
	suite.Require().NoError(err)
	...
	rewards, err = suite.keeper.GetOperatorDelegationRewards(suite.Ctx, delAddr8, operator2.ID)
	suite.Require().NoError(err)
