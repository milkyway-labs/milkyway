...
func (suite *KeeperTestSuite) TestGetAssetAndPrice() {
	...
	_, price, err := suite.keeper.GetAssetAndPrice(ctx, "umilk")
	suite.Require().NoError(err)
	suite.Require().Equal(utils.MustParseDec("2"), price)

	_, price, err = suite.keeper.GetAssetAndPrice(ctx, "uinit")
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestGetCoinValue() {
	...
	value, err := suite.keeper.GetCoinValue(ctx, utils.MustParseCoin("10_000000umilk"))
	suite.Require().NoError(err)
	suite.Require().Equal(utils.MustParseDec("20"), value)

	value, err = suite.keeper.GetCoinValue(ctx, utils.MustParseCoin("10_000000uinit"))
	suite.Require().NoError(err)
	suite.Require().True(value.IsZero())

	value, err = suite.keeper.GetCoinValue(ctx, utils.MustParseCoin("1200000000000000afoo")) // 0.0012 $FOO
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestGetCoinsValue() {
	...
	value, err := suite.keeper.GetCoinsValue(ctx, utils.MustParseCoins("10_000000umilk,5_500000uinit"))
	suite.Require().NoError(err)
	suite.Require().Equal(utils.MustParseDec("36.5"), value)

	value, err = suite.keeper.GetCoinsValue(ctx, utils.MustParseCoins("10_000000umilk,10_000000uatom"))
	suite.Require().NoError(err)
	suite.Require().Equal(utils.MustParseDec("20"), value)
}
