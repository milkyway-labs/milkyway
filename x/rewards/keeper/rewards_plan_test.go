...
func (suite *KeeperTestSuite) TestCreateRewardsPlan_PoolOrOperatorNotFound() {
	...
	_, err := rewardsMsgServer.CreateRewardsPlan(suite.Ctx, types.NewMsgCreateRewardsPlan(
	...
	suite.DelegatePool(utils.MustParseCoin("100_000000umilk"), testutil.TestAddress(1).String(), true)
	suite.DelegatePool(utils.MustParseCoin("100_000000uinit"), testutil.TestAddress(2).String(), true)
