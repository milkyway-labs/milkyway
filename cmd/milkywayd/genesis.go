...
func initializeGenesis(
	...
	// Step 6 -------------------------------------------------------------------------------------------
	// Modify epoch durations for testnet
	log.Info("modifying epoch durations")

	epochsState, err := modifyEpochsDurations(cdc, genesisAppState)
	if err != nil {
		return nil, errors.Wrap(err, "failed to modify epoch durations")
	}

	genesisAppState[epochstypes.ModuleName] = cdc.MustMarshalJSON(epochsState)
...
func modifyEpochsDurations(cdc codec.Codec, genesisAppState map[string]json.RawMessage) (
	*epochstypes.GenesisState,
	error,
) {
	var epochsState epochstypes.GenesisState
	if genesisAppState[epochstypes.ModuleName] != nil {
		cdc.MustUnmarshalJSON(genesisAppState[epochstypes.ModuleName], &epochsState)
	}
	// divide all durations by 6
	epochsState.Epochs = []epochstypes.EpochInfo{
		{
			Identifier:              epochstypes.WEEK_EPOCH,
			StartTime:               time.Time{},
			Duration:                time.Hour * 4 * 7,
			CurrentEpoch:            0,
		...
		{
			Identifier:              epochstypes.MINT_EPOCH,
			StartTime:               time.Time{},
